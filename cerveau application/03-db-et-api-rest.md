...
- **Cadence usine et remise en goulotte** (migration **014** ; cloud : `docker/db/cloud/cloud-12-cadence-usine.sql`, à jouer dans le **Studio en ligne**, d'abord entre `begin;` et `rollback;` car il **modifie des données**) : `horaires_modeles` (niveau × jour × créneau × partie, `debut`/`fin` « HH:MM », `actif` ; 65 lignes amorcées ; contraintes dimanche fermé et samedi sans nuit `NOT VALID` ; droits lecture + ajout + mise à jour, **jamais de suppression**), `cadence_site` (historique **en ajout seul**, `depuis` = décision, `effet` = jour d'application ; amorce Moyen par site), `bons_de_travail.remis_goulotte_le` et `recu_le` ; **données** : process des machines d'activité OAS → `est_oas`, `activite = 'OAS'`, poste de la machine, taux vidé (garde-fou : rien si un nom n'est pas un traitement de surface). API : `/api/production/cadence…` (famille `production`).
- **Mise en stock, types d'objet, zones, BC de reliquat** (migration **013** ; cloud : `docker/db/cloud/cloud-11-mise-en-stock.sql`, à jouer dans le **Studio en ligne** après `cloud-6` et `cloud-10`) : `mises_en_stock` (file `a_ranger` / `range` / `annule`, origines `pv` / `decision_qualite` / `excedent_valide` / `entree_manuelle`, **4 index uniques anti-doublon**, pas de DELETE), `stock_types_objet` (6 types amorcés, modifiables), `stock_zones` (aucune zone amorcée, unique par type sans casse), `stock_emplacements_historique` (**ajout seul** : lecture + ajout), `stock.type_objet` + `ux_stock_reference` (créé seulement sans doublons, sinon WARNING), `mouvements_stock.mise_en_stock_id` (unique), `bons_de_commande.bc_parent_id` / `date_a_valider`, `quarantaines.ligne_idx`. `pv_controle.detail` passe en **v2** (quantités par ligne, nature, identifiants créés, `quarantaines_attendues`) sans DDL. ⚠ Le stock n'est crédité qu'au rangement ; ⚠ `stock.emplacement` = emplacement **fixe** (libellé de zone).
