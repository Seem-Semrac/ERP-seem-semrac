...
--
-- 008 · Décision sur un lot fournisseur non conforme + registre des avoirs fournisseurs
-- (report de docker/db/migrations/008-reception-fournisseur-avoirs.sql — une base neuve
...
-- (report de docker/db/migrations/011-bdt-temps-reglage.sql — une base neuve
--  naît déjà à jour, le lanceur de migrations rejouera le fichier sans effet)
--
...
begin
  ...
  alter table public.bons_de_travail add column if not exists temps_reglage numeric;               -- h ; null = inconnu ; réalisation = duree - temps_reglage
  alter table public.bons_de_travail add column if not exists duree_avant_decoupe numeric;         -- h ; racine avant sa 1re découpe
  alter table public.bons_de_travail add column if not exists temps_machine_avant_decoupe numeric; -- h
...
-- (report de docker/db/migrations/010-index-uniques-upsert.sql — une base neuve naît déjà à jour, le lanceur de
--  migrations rejouera le fichier sans effet). Sans eux : 42P10 à l'enregistrement d'une présence opérateur, d'une
--  affectation aux postes, d'une compétence, d'un article du catalogue fournisseurs ou d'une cible KPI.
--
...
-- (report de docker/db/migrations/013-mise-en-stock.sql — une base neuve naît déjà à jour,
--  le lanceur de migrations la rejoue sans effet)
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if doublons > 0 then
    raise warning '013 : % references de stock sont en double : index unique ux_stock_reference NON cree (rien n est efface).', doublons;
    return;
  end if;
...
begin
  ...
  if to_regclass('public.bons_de_commande') is not null then
  ...
  if to_regclass('public.quarantaines') is not null then
