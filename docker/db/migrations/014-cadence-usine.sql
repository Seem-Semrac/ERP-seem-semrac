-- ═══════════════════════════════════════════════════════════════════════════
-- 014 · Cadence usine par site (modèles d'horaires Bas / Moyen / Haut), remise en goulotte, process OAS — lot G (16/09/2026)
--
-- Demande : « modèles d'horaires de l'usine par cadence (Bas / Moyen / Haut), à adapter selon la cadence choisie. Cadence
-- par site (Seem, Semrac), modifiable à tout moment. Le dimanche est sauté, le samedi selon la cadence. » + « les 5 process
-- de la machine OAS : les marquer OAS et ils doivent être rattachés au service de l'OAS ».
--
-- 1) horaires_modeles (id, niveau bas|moyen|haut, jour_semaine 1=lundi…7=dimanche, creneau matin|apmidi|soir|journee,
--    partie 1|2 (2 = Journée après la pause), debut / fin 'HH:MM', actif, maj_le, maj_par) ; unique (niveau, jour_semaine,
--    creneau, partie). Un créneau FERMÉ un jour = pas de ligne active (une ligne ne s'efface pas : actif = false).
--    Une fin inférieure au début = créneau qui finit le lendemain (Soirée 21:00-05:30), rattaché au jour où il commence.
--    Le dimanche ne peut pas porter de ligne active (contrainte). AMORCÉE avec les modèles fournis le 16/09/2026 (65 lignes,
--    insert … on conflict do nothing : une ligne modifiée ou fermée à l'écran n'est jamais réécrite).
--    ⚠ Haut, jeudi et vendredi : le modèle fourni répétait 05:30-13:15 pour l'Après-midi et la Soirée ; l'utilisateur a
--    confirmé que ces équipes travaillent : amorcés avec les horaires du lundi (13:15-21:00, 21:00-05:30), modifiables.
-- 2) cadence_site (id, site Seem|Semrac, niveau, depuis, effet, par, motif) : historique en AJOUT SEUL (droits lecture +
--    ajout) ; `effet` = jour à partir duquel la cadence s'applique (NULL = jour de `depuis`) ; cadence d'un site à une date
--    = ligne de plus grand (effet, depuis) avec effet ≤ date. Amorce : Moyen pour chaque site qui n'a encore aucune ligne.
-- 3) bons_de_travail.remis_goulotte_le timestamptz : BDT déprogrammé (remis en goulotte) par le déplacement d'une étape
--    précédente (G4) ; la goulotte les affiche en premier. bons_de_travail.recu_le timestamptz : instant de la réception
--    (debut_reel n'est qu'une heure : la date réelle d'un BDT reçu après minuit était perdue).
--    horaires_modeles : un créneau du samedi ne finit pas le lendemain (dimanche toujours fermé), contrainte NOT VALID.
-- 4) Process OAS (données) : les process rattachés à une MACHINE d'activité OAS (ex. MACH-2026-020 « Palan OAS ») sont
--    marqués est_oas = true, activite = 'OAS', poste = poste de cette machine (s'il existe), taux_horaire_machine = null
--    (un process OAS est chiffré au prix, jamais au temps). Critère AMBIGU (un de ces process ne porte pas un nom de
--    traitement de surface : Surtec, désoxydation, oxydation, lavage, OAS) ⇒ RIEN n'est modifié, WARNING avec la liste
--    (action manuelle : Production › Process Ateliers, crayon du process, type OAS). Chaque modification est listée
--    par un NOTICE (avant → après).
--
-- Additive et idempotente. Équivalent cloud : docker/db/cloud/cloud-12-cadence-usine.sql
-- ═══════════════════════════════════════════════════════════════════════════
