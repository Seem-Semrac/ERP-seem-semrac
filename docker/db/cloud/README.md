...
| `cloud-12-cadence-usine.sql` | **à faire** — équivalent de la migration 014 | lot G Production · crée les **modèles d'horaires** `horaires_modeles` (niveau Bas / Moyen / Haut × jour × créneau Matin / Après-midi / Soirée / Journée en deux parties ; dimanche toujours fermé par contrainte ; une ligne ne s'efface pas, elle se ferme) **amorcés** avec les 65 lignes fournies le 16/09/2026 (Haut jeudi et vendredi : Après-midi 13:15-21:00 et Soirée 21:00-05:30, horaires du lundi — le modèle fourni répétait 05:30-13:15, à vérifier à l'écran), la **cadence par site** `cadence_site` (historique en ajout seul, avec sa **date d'effet** `effet` ; **Moyen** amorcé pour Seem et Semrac s'ils n'ont aucune ligne), `bons_de_travail.remis_goulotte_le` et `bons_de_travail.recu_le` (instant de réception d'un BDT) · un créneau du samedi ne peut plus finir le lendemain (contrainte `NOT VALID`) · ⚠ **modifie des données** : les process rattachés à une **machine d'activité OAS** deviennent `est_oas = true`, `activite = 'OAS'`, poste = poste de la machine, `taux_horaire_machine = null` — listés un par un par des NOTICE (onglet « Messages ») ; si l'un d'eux ne porte pas un nom de traitement de surface, **rien** n'est modifié (WARNING, marquage à la main : Process Ateliers › crayon › type OAS). Aperçu sans écrire : jouer le script entre `begin;` et `rollback;`. Sans lui, Production › Process Ateliers › **Cadence usine** affiche « jouez cloud-12 » et n'écrit rien ; présence, planning d'affectation et RH gardent les horaires par défaut des créneaux (aucun créneau refusé) |
| `cloud-13-catalogue-qte-paquet.sql` | **à faire** — équivalent de la migration 017 | lot H1 Catalogue · ajoute `produits_fournisseurs.qte_paquet` (numeric, `> 0` en contrainte `NOT VALID`) : le nombre de **pièces par paquet** du couple (fournisseur, référence), qui ne se déclarait jusque-là que dans chaque nomenclature. La colonne texte `conditionnement` est conservée (libellé libre, repli en lecture). ⚠ **modifie des données** sans rien écraser : la qté par paquet des nomenclatures est recopiée sur la ligne catalogue de même référence (normalisée) ET même nom de fournisseur, uniquement si celle-ci est vide et si la nomenclature n'en donne qu'une seule — chaque copie est listée par un NOTICE (onglet « Messages »). Aperçu sans écrire : jouer le script entre `begin;` et `rollback;`. Crée aussi l'index unique `ux_produits_fournisseurs_ref_norm (fournisseur_id, lower(btrim(reference)))`, **seulement sans doublons** (sinon WARNING avec la requête pour les lister, rien n'est effacé) ; `ux_produits_fournisseurs_upsert` (cloud-8) reste en place. **Sans lui** : aucune page cassée — le prix d'un accessoire est estimé en comptant « 1 pièce par paquet » et affiché en orange (« conditionnement non déclaré chez X »), et l'enregistrement d'une qté par paquet depuis le catalogue ou la validation d'une demande de prix est accepté puis **signalé** comme non conservé (« jouez cloud-13 ») |
| `cloud-14-lots-sous-lots.sql` | **à faire, APRÈS cloud-13** — équivalent de la migration 018 | lot H2 Production · **sous-lots des nomenclatures mères** : ajoute à `lots` `lot_parent` (id du lot parent, NULL = racine, sans clé étrangère), `rang` (ordre de fabrication parmi les frères ; racine = son n° ZZ), `niveau` (0 = racine … 4, NOT NULL DEFAULT 0 : les lots existants restent des racines), `nomenclature_id` (nomenclature validée retenue au lancement) et `qte_par_parent` ; index `ix_lots_lot_parent` ; contraintes `NOT VALID` (niveau 0 à 4, rang ≥ 1, un lot n'est pas son propre parent) · purement additif, **aucune donnée modifiée**. **Sans lui** : aucune page cassée — une pièce mère est lancée **sans sous-lots**, comme avant, avec un avertissement à l'acceptation de l'offre et sur la fiche du lot ; « Créer les sous-lots » répond 409 « jouez cloud-14 » et n'écrit rien ; l'ordre des composants, la mère dans la mère et les refus de cycle / profondeur fonctionnent déjà (jsonb). **Ensuite** : sur la fiche du lot racine de chaque pièce mère lancée avant, « Créer les sous-lots » (aperçu, puis création ; rejouable sans doublon) |

**Migrations Docker sans script cloud (lot H0, 17/09/2026)** — rien à jouer en ligne :

- `015-nomenclature-composants-jsonb` : `nomenclatures.composants` est **déjà en `jsonb`** sur le cloud (sondé en lecture seule le 17/09/2026 : le filtre REST `composants=cs.[]` est accepté, il est refusé sur une colonne `text`). Seules les bases Docker / VM nées de `schema.sql` l'avaient en `text`.
- `016-ged-bucket-policies` : le cloud a **déjà** son bucket privé `ged` et ses policies `ged_*`.
