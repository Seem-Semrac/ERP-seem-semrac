...
export function pageAffaireFiche(num: string, detail?: any): string {
  ...
  const lotRows = allLots.map((l: any) => `<tr style="border-bottom:1px solid #f9fafb;">${td(`<span style="font-family:monospace;font-weight:700;">${esc(l.id)}</span>`)}${td(esc(l.piece || '—'))}${td(l.qte || 0, 'center')}${td(statusBadge(l.statut || ''), 'center')}${td(l.nc ? `<span style="color:#dc2626;font-weight:700;">${l.nc} NC</span>` : '—', 'center')}</tr>`)
  ...
  <div style="padding:20px 24px;max-width:1100px;margin:0 auto;">
    <a href="/commercial/service#affaires" style="display:inline-flex;align-items:center;gap:6px;color:${ACC};text-decoration:none;font-weight:700;font-size:.8rem;margin-bottom:14px;"><i class="fas fa-arrow-left"></i> Retour aux affaires</a>
      ...
      ${miniTable('Production — lots', 'fa-layer-group', '#14b8a6', [{ t: 'Lot' }, { t: 'Pièce' }, { t: 'Qté', a: 'center' }, { t: 'Statut', a: 'center' }, { t: 'NC', a: 'center' }], lotRows, 'Aucun lot en production.', bdtTotal ? (bdtSoldes + '/' + bdtTotal + ' BDT soldés') : '')}
