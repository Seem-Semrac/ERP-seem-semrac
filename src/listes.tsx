...
export const pagePreparationsTechniques = (dbPreps?: any[]) => {
  ...
  const prepStatut = (s: string) => {
    const m: Record<string,{bg:string,col:string,lbl:string}> = {
      ...
      realise: {bg:'#dcfce7',col:'#166534',lbl:'Réalisé ✓'},
    }
  ...
  <div style="background:linear-gradient(135deg,#0891b2,#0e7490);padding:20px 24px;color:white;">
    <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
      <div style="background:rgba(255,255,255,.15);border-radius:10px;padding:8px 12px;"><i class="fas fa-tools" style="font-size:1.2rem;"></i></div>
      <div>
        <h1 style="font-size:1.1rem;font-weight:800;margin:0;">Préparations Techniques</h1>
        <p style="font-size:.78rem;opacity:.8;margin:2px 0 0;">BE · Gammes · Programmes CN · Plans de contrôle · First Article · EN9100</p>
      </div>
    </div>
  </div>
  <div style="padding:22px 30px;">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px;">
      ${[['À faire',nbAFaire,'#ef4444'],['En cours',nbEnCours,'#3b82f6'],['Réalisées',nbRealises,'#22c55e'],['Total',nbTotal,'#6b7280']].map(([l,v,c])=>'<div style="background:white;border-radius:12px;padding:16px;box-shadow:0 1px 3px rgba(0,0,0,.07);border-top:3px solid '+c+';text-align:center;"><div style="font-size:2rem;font-weight:800;color:'+c+';">'+v+'</div><div style="font-size:.72rem;color:#6b7280;font-weight:600;">'+l+'</div></div>').join('')}
    </div>
    ...
    <div style="background:white;border-radius:14px;box-shadow:0 1px 3px rgba(0,0,0,.07);overflow:hidden;">
      <table style="width:100%;border-collapse:collapse;font-size:.8rem;" id="prepTable">
        <thead><tr style="background:#f8fafc;border-bottom:2px solid #f1f5f9;">
          ...
          <th style="text-align:center;padding:10px 14px;color:#64748b;font-size:.68rem;text-transform:uppercase;font-weight:700;">Échéance</th>
          <th style="text-align:center;padding:10px 14px;color:#64748b;font-size:.68rem;text-transform:uppercase;font-weight:700;">Statut</th>
          <th style="text-align:center;padding:10px 14px;color:#64748b;font-size:.68rem;text-transform:uppercase;font-weight:700;">Action</th>
        </tr></thead>
        <tbody>
  ...
  <script>
  function filtrerPrep(s){
    document.querySelectorAll('[data-prepf]').forEach(function(b){
      if(b.tagName==='BUTTON'){ b.style.background='#f1f5f9'; b.style.color='#6b7280'; b.style.borderColor='#e2e8f0'; }
    });
  ...
  function marquerFait(id){
    fetch('/api/prepa-technique/'+encodeURIComponent(id)+'/statut',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({statut:'faite'})})
      .then(function(r){return r.json();}).then(function(j){
        if(!j||!j.ok){ if(typeof pushNotif!=='undefined') pushNotif('err','fa-times',(j&&j.error)||'Échec.',4000); return; }
        if(typeof pushNotif!=='undefined') pushNotif('ok','fa-check','Préparation '+id+' réalisée.',3500);
        if(typeof softReload!=='undefined'){ setTimeout(softReload,600); } else { setTimeout(function(){location.reload();},600); }
      }).catch(function(){ if(typeof pushNotif!=='undefined') pushNotif('err','fa-times','Erreur réseau.',4000); });
  }
  </script>`

  return layout('Préparations Techniques', content, 'prep-tech')
}
