...
const M = [
  ...
  { svc:'achats', label:'Achats', route:'/achats/service', file:'src/achats.tsx', rw:'achats, stock', r:'commercial, be, production, expéditions, maintenance',
    ...
    tabs:[['rfq','Demandes de prix'],['bc','Bons de commande'],['avoirs','Avoirs fournisseurs'],['da','Demandes d\'achat'],['fourn','Fournisseurs / ST'],['scorecard','Scorecard'],['dashboard','Dashboard achats']],
    api:['achats','da','bc','avoirs-fournisseurs','fournisseur(s)','sous-traitant(s)','demandes-prix','catalogue-fournisseurs'], tables:['fournisseurs','sous_traitants','demandes_prix','demandes_achat','bons_de_commande','avoirs_fournisseurs','factures_fournisseur'],
  ...
  { svc:'production', label:'Production', route:'/production/service', file:'src/prod.tsx', rw:'production', r:'be, achats, oas, qualité, sécurité, expéditions, stock, maintenance',
    ...
    api:['production','bdt','bds','planning','process','machine(s)','presence','cadence','sortie-matiere'], tables:['commandes','lots','bons_de_travail','bons_sous_traitance','machines','postes','shifts','absences','operateur_presence','affectation_poste','process_atelier','horaires_modeles','cadence_site'],
    notes:'Planning unique dans /production/service : file « BDT à classer » et « BDS à planifier » AU-DESSUS de chaque Gantt ; clic sur un BDT/BDS = focus étape (son poste/sous-traitant + étape précédente et suivante). BDT/BDS prioritaires (id BDTP/BDSP) encadrés rouge. Lot H2 : le lot d\'une pièce mère porte un sous-lot par composant (LOT-…-01.02, récursif), programmable comme le reste (vigilance « sous-lots non terminés » sur la mère) ; libération, BL et KPI sur les lots racines (section notes écrite à la main : voir « Lot H2 » dans la fiche).' },
  { svc:'oas', label:'OAS', route:'/oas/service', file:'src/oas.tsx', rw:'oas', r:'be, production, qualité, sécurité, stock, maintenance',
    ...
    notes:'Process « OAS » = porte de lot (soldage BDT précédent → OAS → BDT suivant). Un bain non conforme génère une NC. Lot G (16/09/2026) : section « Lots à venir » (lots dont l’étape qui précède l’OAS est programmée ou reçue au planning, fin prévue en heures ouvrées de la cadence usine) ; les process de la machine OAS portent est_oas = true et l’activité « OAS » (migration 014 / cloud-12).' },
