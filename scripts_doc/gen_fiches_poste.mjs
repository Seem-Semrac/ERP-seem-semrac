...
const R = [
  ...
  { role: 'qualite', label: 'Qualiticien(ne)', rw: ['qualité', 'sécurité'], r: ['be', 'production', 'oas', 'expéditions', 'stock', 'maintenance'],
    ...
    taches: [
      ...
      ['Décider d\'un lot reçu non conforme', 'qualite-quarantaine', ['Qualité › **Quarantaine** : un lot refusé au contrôle réception porte le bouton rouge **Décider** (les lots de production gardent « Statuer »). Le motif et la NC (détecteur « Réception ») reprennent le PV ligne par ligne : une ligne par écart, certificat matière absent, contrôleur.', 'Trois choix : **renvoyer** tout le lot au fournisseur · **dérogation** (il entre en stock en l\'état, réfaction possible) · **entrée partielle** (la part acceptée entre, le reste repart ou part au rebut).', 'Renvoi et entrée partielle : choisir **avoir** (il part aux Achats) ou **remplacement** (le fournisseur relivre).', 'Le **motif est obligatoire** et la décision ne se prend qu\'une fois. Matière renvoyée sans remplacement = il faut repasser commande.']],
  ...
  { role: 'logistique', label: 'Logistique (expéditions & stock)', rw: ['expéditions', 'stock'], r: ['commercial', 'achats', 'production'],
    ...
    taches: [
      ...
      ['Réceptionner une commande fournisseur', 'form-expeditions-reception', ['Expéditions › **Calendrier**, bloc « à réceptionner aujourd\'hui » : **Traiter** sur la ligne du bon de commande.', 'Saisir le **N° de commande fournisseur** et le **N° de BL fournisseur** (obligatoires). Pas de quantité : c\'est le reste à recevoir du BC (sauf « Livraison partielle » ou BC sans quantité).', '**Réception hors France ?** Oui / Non. Si Oui : poids matière (kg), N° de nomenclature (douane), code EWX (1 ou 2), mode d\'arrivée.', 'Valider : la réception passe dans l\'onglet **Réceptions** (« PV à faire »). Faute de frappe → lien **Corriger**.']],
      ['Faire le PV de contrôle à réception', 'form-expeditions-pv-non-conforme', ['Expéditions › **Réceptions**, bouton **PV à faire**.', 'Choisir **Conforme** ou **Non conforme** et le **contrôleur** (personnes ayant l\'écriture Expéditions). BC exigeant un certificat matière : cocher « Certificat matière reçu et conforme » — sans elle, pas de conforme.', 'Conforme : le contenu entre en stock. Non conforme : **Oui / Non** sur chaque ligne du BC, **observation obligatoire** sur chaque Non, observation générale, gravité → NC en Qualité et lot en quarantaine.']],
