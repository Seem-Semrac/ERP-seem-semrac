...
| Que fait-on de ce lot ? | choix (Renvoyer au fournisseur, Dérogation avec le fournisseur, Entrée partielle en stock) | Oui | « Renvoyer » : tout repart, rien n'entre en stock. « Dérogation » : le lot est accepté en l'état et part dans Stock › Mise en stock (crédité au rangement). « Entrée partielle » : après tri, la part acceptée part dans Stock › Mise en stock. | Entrée partielle en stock |
| Part acceptée par ligne du bon de commande | un nombre par ligne (bloc bleu, visible si le lot couvre plusieurs articles) | Oui pour une entrée partielle sur plusieurs articles ; pré-rempli en dérogation | Ce qui est accepté sur **chaque** ligne (0 sur une ligne dont rien n'est gardé), sans dépasser sa quantité en quarantaine. Chaque ligne part en Mise en stock sous sa référence. | Ligne 1 : 8 · Ligne 2 : 0 |
| Quantité acceptée | nombre (lecture seule = somme des lignes quand le lot couvre plusieurs articles) | Oui (entrée partielle) | Ce qui part en Mise en stock. Le reste s'affiche à côté. Doit être inférieur à la quantité du lot — tout accepter, c'est une dérogation. | 6 |
...
💡 **Astuce :** Après validation, la ligne affiche ce qui a été fait (« Renvoyé au fournisseur », « Dérogation fournisseur », « Entrée partielle 6/10 ») et ce qui reste à faire (« Retour à expédier », « Part acceptée → Stock › Mise en stock », « Avoir réclamé (Achats) », « Remplacement attendu »).
⚠️ **Attention :** La décision ne se prend **qu'une fois** — un second clic est refusé. Si la matière est renvoyée **sans remplacement**, un message prévient qu'elle manque à l'affaire : il faut **repasser commande**. Si la base n'a pas encore la mise à jour (migration 008 / script cloud-6), la décision est refusée. Sur plusieurs articles, une entrée partielle sans part par ligne (ou dont la somme ne fait pas la quantité acceptée) est refusée **sans consommer la décision**.
