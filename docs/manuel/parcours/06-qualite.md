...
**⚠️ Réception fournisseur :** le PV d'une livraison fournisseur ne se saisit pas ici mais aux **Expéditions** (onglet Réceptions › « PV à faire »), ligne par ligne sur le bon de commande, par un contrôleur ayant l'**écriture sur les Expéditions**. Depuis le 14/09/2026, un compte Qualité ne peut plus le signer, sauf si sa fiche porte l'écriture Expéditions (en gardant cochés ses autres services). Depuis le 15/09/2026, le PV saisit la **quantité reçue de chaque ligne** et arrive ici tout seul : **une NC fournisseur par ligne à problème** (quantitative « Logistique - Quantité », qualitative « À requalifier » ou les deux, détecteur « Réception ») et, pour chaque ligne **avec observation** (ou toute la réception si le certificat matière manque), une **quarantaine** à décider par le bouton « Décider » (Quarantaine). La part acceptée (dérogation, entrée partielle — répartie **par ligne** quand le lot couvre plusieurs articles) part dans **Stock › Rangement / Mise en stock** : le stock est crédité au rangement.
