...
**`nomenclatures.composants` en `jsonb` (lot H0, 17/09/2026 — migration Docker `015-nomenclature-composants-jsonb.sql`, aucun script cloud)** — liste des standards d'une **mère** : tableau `[{nom_id, num_nom, code, prix, qte}]`. Le cloud l'a toujours eue en `jsonb` (sonde en lecture du 17/09/2026 : le filtre REST `composants=cs.[]` y est accepté, alors qu'il est refusé en 42883 sur une colonne `text` ou `numeric`) ; la base Docker / VM, née de `schema.sql` (miroir d'une colonne vide), l'avait en **`text`** : PostgREST rendait une chaîne JSON, le formulaire BE n'y voyait aucun composant et l'enregistrement suivant écrasait la mère avec `[]`. 015 convertit (vide / NULL → `[]`, JSON valide → cast à l'identique, rien d'effacé) et `schema.sql` porte désormais `jsonb`. Quel que soit le type, l'application lit la colonne par `composantsDe()` (`src/shared.ts`, toujours un tableau) et `PUT /api/nomenclature/:id` refuse (409 `composants_vides`) de vider une liste non vide sans `vider_composants: true`. Au passage : `bons_de_livraison.lignes` est aussi en `jsonb` sur le cloud.

**Catalogue `produits_fournisseurs` — règles d'écriture (lot H0)** : aucune colonne ajoutée. Une ligne **existante** n'est plus jamais réécrite en bloc : l'enregistrement d'une nomenclature n'insère qu'un couple absent (sans prix, `statut = 'en_attente_prix'`, `source_prix = 'be'`) ; « Déclarer » / « Éditer » ne mettent à jour que les champs fournis et non vides (prix > 0 seulement, `categorie` / `activite` jamais réécrites par « Déclarer » sauf vides en base, prix inchangé = `date_prix` et `source_prix` conservées) ; la validation d'une demande de prix n'écrit sur une ligne existante que `prix`, `devise`, `date_prix`, `source_prix = 'rfq'`, `statut`, et n'ajoute à `ref_prix_historique` qu'**un** point par (référence, fournisseur, n° de demande `bc_num`, prix). Un prix ≤ 0 n'est jamais écrit comme prix officiel. Fraîcheur d'un prix : **183 jours** (`PRIX_VALIDITE_JOURS`).

**Catalogue `produits_fournisseurs` — quantité par paquet et unicité de la référence (lot H1, 17/09/2026 — migration Docker `017-catalogue-qte-paquet.sql`, cloud `cloud-13-catalogue-qte-paquet.sql`)** :

- **`+ qte_paquet numeric`** (contrainte `> 0` `NOT VALID`) — nombre de **PIÈCES par paquet** du couple (fournisseur, référence). Le prix catalogue d'un accessoire reste le **prix du PAQUET** ; le prix à la pièce est un **calcul**, jamais une donnée stockée (`prix / qte_paquet`, `src/prix_moyen.ts`). `NULL` = conditionnement **non déclaré** : le prix est compté à la pièce **et signalé** en orange. La colonne texte **`conditionnement`** est **conservée** (libellé libre) et sert de repli en lecture — elle n'est plus écrite par personne.
- **Reprise sans écrasement** : la quantité par paquet ne se saisissait jusque-là que ligne par ligne dans les nomenclatures (`fournitures_nomenclature.qte_paquet`). 017 la recopie sur la ligne catalogue de **même référence normalisée ET même nom de fournisseur**, uniquement si celle-ci est vide et si la nomenclature n'en donne qu'**une seule** (chaque copie listée par un NOTICE). Sur le Docker : 1 ligne reprise (`136290` / BOSSARD = 100) — sans elle, cet accessoire passait de 0,104 € à **10,40 € la pièce** au premier enregistrement au nouveau modèle.
- **`+ demandes_prix_reponses.qte_paquet numeric`** (contrainte `> 0` `NOT VALID`) — pour un accessoire, le conditionnement auquel se rapporte le prix chiffré par **ce** fournisseur, saisi par les Achats à côté du prix et recopié au catalogue à la validation.
- **Index unique `ux_produits_fournisseurs_ref_norm (fournisseur_id, lower(btrim(reference)))`** : la même référence ne peut plus être déclarée **deux fois chez le même fournisseur** à la casse et aux espaces près (la même référence chez **plusieurs** fournisseurs reste normale — c'est la base de la moyenne). Créé **seulement sans doublons** (0 doublon sur 144 lignes le 17/09/2026) ; sinon WARNING avec la requête pour les lister, rien n'est effacé. `ux_produits_fournisseurs_upsert` (010) **reste en place** : c'est lui qu'exige le `on conflict (fournisseur_id, reference)`. ⚠ L'index SQL ignore la casse et les espaces mais **pas les accents** ; l'unicité **applicative** (`normaliserReference`, 409 `doublon_catalogue`) les ignore aussi et c'est elle qui refuse en premier — passer l'index en `unaccent(...)` exigerait l'extension côté cloud.
- **Écritures** : toute écriture portant `qte_paquet` passe par les helpers **tolérants** de `src/queries.ts` (`updateProduitFournisseurTolerant`, `insertProduitFournisseurSiAbsentTolerant`, `create/updateDemandePrixReponseTolerant`), qui rendent `{ data, error, qte_paquet_ignoree }` ; toutes les **lectures** sont en `select('*')` (jamais de projection, de filtre ni d'`order` nommant `qte_paquet`). Les règles d'écriture partielle du lot H0 sont **conservées** telles quelles.
- **Base sans la colonne** (cloud tant que `cloud-13` n'est pas joué — `42703` confirmé le 17/09/2026) : **aucune page cassée**. La valeur arrive `undefined`, `qtePaquetDe()` retombe sur `conditionnement` puis sur `null` ; une écriture répond **200** avec `avertissement` (« quantité par paquet non enregistrée… jouez `cloud-13` »), jamais 500.

**`fournitures_nomenclature`** (lot H1) : `fournisseur` et `qte_paquet` ne sont **plus écrits** — aucun `DROP`, les colonnes restent et une ancienne ligne se relit telle quelle (réaffichée avec le nouveau calcul, basculée au prochain enregistrement volontaire). Aucune reprise en masse. **Aucun index unique** n'y est posé : l'unicité d'une référence dans une fiche est **applicative** (409 `doublon_reference`, **avant** l'effacement / réinsertion de `upsertFournitures`).

**`fournisseurs.catalogue` / `sous_traitants.catalogue` (jsonb)** : conservés en base, mais **vides partout** (sonde en lecture du 17/09/2026 : cloud 123 `[]` + 32 `null`, Docker idem ; 0 article) et **retirés de la fiche fournisseur**. Le catalogue, c'est `produits_fournisseurs`. Les sous-traitants gardent leur jsonb en lecture (pas de table équivalente).
...
**Sous-lots des pièces mères (lot H2, 18/09/2026 — migration Docker `018-lots-sous-lots.sql`, cloud `cloud-14-lots-sous-lots.sql`, à jouer APRÈS `cloud-13`)** — le lot d'une pièce mère porte un **sous-lot par composant** de sa nomenclature, récursivement ; un sous-lot est une ligne `lots` **ordinaire** (même `cmd_id`, statut, `piece` = code de la fille). Règles : `06-modules/production.md`, section « Lot H2 ».

| Colonne `lots` | Type | Sens |
|---|---|---|
| **`lot_parent`** | text | id du lot parent ; `NULL` = racine. **Sans clé étrangère** (lien souple, comme `lot_ref` : un parent annulé ne doit rien bloquer) ; index `ix_lots_lot_parent` |
| **`rang`** | integer | position parmi les frères = **ordre de fabrication** (haut → bas de la nomenclature mère) ; racine = son n° `ZZ` ; `NULL` = lot antérieur à H2 (le suffixe de l'id fait foi). Contrainte `lots_rang_valide` (`NULL` ou ≥ 1) |
| **`niveau`** | integer `NOT NULL DEFAULT 0` | 0 = racine, 1 = sous-lot, 2 = sous-sous-lot… **4 au plus** (5 niveaux, racine comprise) — contrainte `lots_niveau_valide` ; les lots existants restent des racines |
| **`nomenclature_id`** | text | nomenclature **validée** retenue au lancement (dernière révision validée du code de la pièce) — traçabilité EN 9100 ; la fiche lot et « Créer les sous-lots » la lisent en premier |
| **`qte_par_parent`** | numeric | quantité du composant par unité du parent (`NULL` = racine) ; `qte` du sous-lot = `ceil(qte parent × qte_par_parent)`, minimum 1 |

- Contrainte `lots_parent_distinct` (un lot n'est pas son propre parent). Les 3 contraintes sont **`NOT VALID`** : elles
  contrôlent les écritures à venir sans juger l'existant. Purement **additif** : aucune donnée modifiée.
- **Numérotation** : la racine reste `LOT-YYYY-AFF-ZZ` ; un sous-lot ajoute `.RR` (rang, 2 chiffres) :
  `LOT-2026-0001-01.02`, `LOT-2026-0001-01.02.01` ; BDT / BDS `BDT-2026-0001-01.02-03` (`src/nomenclature_arbre.ts`). Les
  BDT / BDS d'un sous-lot ont le **même `num_affaire`** que la racine, `lot_ref` = id du sous-lot.
- **`nomenclatures.composants`** : **aucune DDL** (jsonb depuis 015) — `rang`, `type_nom`, `indice` vivent dans le jsonb ;
  l'ordre du tableau est l'ordre de fabrication. `parent_id` / `qte_par_mere` restent des colonnes **mortes** (envoyées `null`).
- **Lectures** : toutes en `select('*')` ; la présence des colonnes se teste par `lotsArbreDispo()` (`src/queries.ts` : sonde
  `select` des 5 colonnes, `42703` / `PGRST204` ⇒ absentes, autre erreur ⇒ panne signalée). **Sans elles** (cloud tant que
  `cloud-14` n'est pas joué) : aucune page cassée, `lot_parent` vaut `undefined` et l'arbre se déduit de l'id ; une mère est
  lancée **sans sous-lots** avec un avertissement ; « Créer les sous-lots » répond 409 `arbre_indisponible`.
- ⚠ **Ne jamais construire un `.or()` PostgREST avec des ids de lot** (le point et les tirets des ids) : `eq` / `in` seulement.
...
**Cadence usine, remise en goulotte, réception datée, process OAS (lot G, 16/09/2026 — migration Docker `014-cadence-usine.sql`, cloud `cloud-12-cadence-usine.sql`)** — additive et idempotente ; ⚠ la partie 4 **modifie des données** (process OAS, voir ci-dessous) :
- **`horaires_modeles`** (`id` text pk, `niveau` `bas`/`moyen`/`haut`, `jour_semaine` 1 = lundi … 7 = dimanche, `creneau` `matin`/`apmidi`/`soir`/`journee`, `partie` 1 ou 2 — 2 = Journée après la pause —, `debut` / `fin` texte « HH:MM », `actif` boolean, `maj_le` timestamptz, `maj_par`) : modèles d'horaires de l'usine par niveau de cadence. **Index unique** `ux_horaires_modeles_cle (niveau, jour_semaine, creneau, partie)`. **Contraintes** : `horaires_modeles_niveau_valide`, `_jour_valide`, `_creneau_valide`, `_partie_valide` (partie 2 seulement pour `journee`), `_heures_valides` (regex HH:MM, `debut <> fin`), `_dimanche_ferme` (`jour_semaine <> 7 or actif = false`), `_samedi_sans_nuit` (`jour_semaine <> 6 or actif = false or fin > debut`, **NOT VALID** : contrôle les écritures à venir). Une fin inférieure au début = créneau qui finit le **lendemain**, rattaché au jour où il commence. Un créneau fermé un jour = **aucune ligne active** : une ligne ne s'efface jamais (droits anon/authenticated : `select`, `insert`, `update` — **pas** de `delete`). **Amorce** : les 65 lignes fournies le 16/09/2026 (`id` = `niveau-jour-creneau-partie`, `insert … on conflict do nothing` : une ligne modifiée ou fermée à l'écran n'est jamais réécrite) — Bas 17, Moyen 22, Haut 26 ; Haut jeudi et vendredi : Après-midi et Soirée amorcées avec les horaires du lundi (le modèle fourni répétait 05:30-13:15).
- **`cadence_site`** (`id` text pk, `site` `Seem`/`Semrac`, `niveau`, `depuis` timestamptz = moment de la décision, **`effet` date** = jour à partir duquel la cadence s'applique, `par`, `motif`) : **historique en ajout seul** (droits `select` + `insert` seulement). Cadence d'un site à une date D = la ligne de plus grand (jour d'effet, `depuis`) dont le jour d'effet ≤ D ; jour d'effet = `effet`, sinon le jour de `depuis` à Paris (lignes d'amorce) ; avant la première ligne : la première. Index `idx_cadence_site_depuis (site, depuis desc)`. **Amorce** : `amorce-seem` et `amorce-semrac` en **Moyen** (`par` = « migration 014 ») pour chaque site qui n'a encore aucune ligne.
- `bons_de_travail` **`+ remis_goulotte_le timestamptz`** (BDT déprogrammé parce que le déplacement d'une étape précédente le rendait incohérent ; la goulotte les affiche en premier ; remis à `NULL` quand le BDT est reposé) + index partiel `idx_bdt_remis_goulotte`. **`+ recu_le timestamptz`** = instant de la réception (`POST /api/production/bdt/:id/recu`) : `debut_reel` n'est qu'une heure « HH:MM », et la date réelle d'un BDT reçu après minuit était perdue. `NULL` = reçu avant la colonne (date déduite de la date prévue).
- **Process OAS (données)** : les `process_atelier` rattachés (`machine_id`) à une **machine d'activité OAS** passent `est_oas = true`, `activite = 'OAS'`, `poste_id` = poste de la machine (s'il existe), `taux_horaire_machine = NULL` (écrit seulement si la colonne de `cloud-7` existe). **Garde-fou** : si l'un d'eux ne porte pas un nom de traitement de surface (regex `surtec|désoxyd|oxydation|lavage|oas`), **aucun** process n'est modifié (WARNING avec la liste). Un NOTICE par ligne modifiée (avant → après) ; les process nommés comme un traitement de surface mais hors machine OAS sont listés, pas modifiés. Docker : 5 process (Desoxydation, Lavage, Oxydation Noire, Oxydation Incolore, Surtec 650 — machine `MACH-2026-020` « Palan OAS », poste OAS). ⚠ Conséquence : ces process ont désormais l'activité **« OAS »** (ni Seem ni Semrac) — gamme du BE et filtres RH › Compétences adaptés.
- **Lecture applicative** : `src/cadence_db.ts` (`lireDonneesCadence` : toutes les lignes, `actif = false` comprises, paginées par 1 000). Aucun cache : `/production/service`, `/rh/temps`, `/oas/service`, la présence et chaque pose au planning relisent la cadence.
- **Sans 014** (cloud avant `cloud-12`) : Production › Process Ateliers › Cadence usine affiche « jouez cloud-12 » (routes d'écriture 409 `table_absente`) ; présence, planning d'affectation et RH gardent les horaires par défaut des créneaux (`SHIFTS`) et rien n'est refusé ; le planning reste sur la grille 5 h–23 h sans refus d'heure fermée ; la remise en goulotte fonctionne mais sans `remis_goulotte_le` (avertissement « Colonne remis_goulotte_le absente », BDT pas placés en tête) ; la réception d'un BDT s'écrit sans `recu_le` (réessai automatique) ; un changement de cadence sur une base qui aurait les tables sans la colonne `effet` est refusé (409). Aucune erreur bloquante en lecture.

### OAS (traitement de surface)
Relevés eau/bains, balancelles (via tables production/oas + relevés) — un bain non conforme génère une NC. Lot G (16/09/2026) : les process de la machine OAS portent `est_oas = true` et `activite = 'OAS'` (migration 014 / `cloud-12`, section Production) ; la liste « Lots à venir » n'a pas de table : elle est calculée à la volée depuis `bons_de_travail` (`oas_apres`, statut, jour, heure, `recu_le`), `lots`, `nomenclatures.etapes_production` et la cadence usine.
...
### RH
`salaries` (matricule, PIN haché, rôles, `taux_horaire_charge` = **taux homme** des coûts de revient depuis le 14/09/2026 — moyenne par site pour le coût estimé, taux de l'opérateur pour le coût réel d'un BDT —, `est_operateur`), `habilitations`, `certifications`, `conges`, `pointages`, `formations`. Lot G (16/09/2026) : RH › Gestion des temps lit `horaires_modeles` et `cadence_site` pour les horaires des présences (site = activité de l'opérateur).

### Maintenance
`ordres_maintenance` (OM), `plans_preventif`, `pieces_detachees`, historique/MTBF.

### GED / Plan bâtiment
Bucket privé `ged` et policies `ged_*` : sur le cloud, déjà en place ; sur Docker / VM, créés par la migration **016** (lot H0 — une base neuve n'avait ni l'un ni les autres, voir `12-docker-installation.md`). `documents.fichier_nom` garde le nom **exact** du fichier (caractères au-delà de Latin-1 compris) : c'est `GET /api/ged/file/:id` qui l'encode dans l'en-tête (`filename*=UTF-8''…`).
...
- **014** cadence usine (lot G) : `horaires_modeles` (65 lignes amorcées, contraintes dont dimanche fermé et samedi sans nuit `NOT VALID`), `cadence_site` (historique en ajout seul, colonne `effet`, amorce Moyen par site), `bons_de_travail.remis_goulotte_le` + `recu_le`, **données** : process des machines d'activité OAS marqués `est_oas` / `activite = 'OAS'` (garde-fou si le critère est ambigu) — cloud : `cloud-12`. Sans elle, horaires par défaut partout, planning sur la grille 5 h–23 h, écran Cadence usine « jouez cloud-12 ».
- **015** `nomenclatures.composants` `text` → `jsonb` (lot H0) — **pas de script cloud** (le cloud est déjà en `jsonb` ; aucun `cloud-13` n'a été écrit). Lève une **exception** — transaction annulée, fichier non journalisé, retenté au démarrage suivant — si la conversion est impossible : verrou tenu plus de 5 s (55P03), droits (42501), dépendance, ou valeur non JSON (requête de diagnostic dans le message, correction à la main ; rien n'est jamais remplacé). Sans elle, les composants restent lisibles (`composantsDe`) et protégés (409), mais la colonne reste `text`. ⚠ Sur le Docker local, 015 a été journalisée le 17/09/2026 dans sa **première** version (erreurs en NOTICE) ; la version finale ne change que le traitement des erreurs et la colonne y est bien en `jsonb` : empreinte différente, rien à rejouer.
- **016** bucket privé `ged` + policies `ged_*` (contenu de `db/seed/storage_policies.sql`) — **Docker seul**, le cloud les a déjà. Lève une exception si `storage.buckets` / `storage.objects` n'existent pas encore (storage-api pas encore passé sur une base neuve) : retentée au démarrage suivant. Volontairement **sans** `depends_on: storage` (un `erp-storage` « unhealthy » bloquerait toutes les migrations). Sans elle, sur une base neuve : dépôt GED refusé (« Bucket not found » / « row-level security policy »).
- **017** catalogue fournisseurs : quantité par paquet (lot H1) — `produits_fournisseurs.qte_paquet`, `demandes_prix_reponses.qte_paquet`, index unique `ux_produits_fournisseurs_ref_norm`, **reprise** de la qté/paquet des nomenclatures vers le catalogue (sans écrasement) — cloud : **`cloud-13`, à jouer à la main dans Supabase Studio**. Additive et idempotente ; testée à blanc deux fois sur le Docker local (`begin; … rollback;`). Sans elle : le prix d'un accessoire est estimé en comptant « 1 pièce par paquet » et signalé en orange, et l'enregistrement d'une quantité par paquet est accepté puis annoncé comme non conservé (voir la section Bureau d'études ci-dessus).
- **018** sous-lots des pièces mères (lot H2) — `lots.lot_parent`, `rang`, `niveau` (`NOT NULL DEFAULT 0`), `nomenclature_id`, `qte_par_parent`, index `ix_lots_lot_parent`, contraintes `NOT VALID` `lots_niveau_valide` / `lots_rang_valide` / `lots_parent_distinct` — cloud : **`cloud-14`, à jouer à la main dans Supabase Studio APRÈS `cloud-13`**. Purement additive et idempotente (aucune donnée modifiée) ; testée à blanc deux fois sur le Docker local (`begin; … rollback;`, contraintes éprouvées : niveau 5, rang 0 et parent = soi refusés), puis appliquée et journalisée par `erp-docker.sh maj` le 18/09/2026. Sans elle : une pièce mère est lancée sans sous-lots, avec un avertissement (voir la section Production ci-dessus).
...
PostgREST refuse **toute la requête** (PGRST204) : le lot n'est donc **jamais marqué « terminé » en production**, et l'erreur passe inaperçue car le retour n'est pas contrôlé. Vérifié par sonde `PATCH` sur un id inexistant : HTTP 400. **Jouer `db_conformite_cloud.sql` corrige ce bug.** Depuis le lot H2 (18/09/2026), `recomputeCmdAvancement` **retente l'écriture sans `updated_at`** quand la base ne l'a pas : le lot passe « terminé » même sans le script (qui reste à jouer pour la parité).
