...
- `bons_de_livraison.qte` d'une réception = **reste à recevoir** du BC, ou quantité livrée saisie (livraison partielle, quantité commandée inconnue) ; **`NULL` = inconnue, jamais 0**. `transport` / `transporteur_ref` ne sont plus écrits par la réception (colonnes conservées : BL clients, historique).
